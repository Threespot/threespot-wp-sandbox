...
Copyright 2017-2024 SearchWP, LLC
...
if ( ! defined( 'SEARCHWP_EMR_VERSION' ) ) {
	define( 'SEARCHWP_EMR_VERSION', '1.1.3' );
}
...
class SearchWP_EMR_Integration {
	...
	function maybe_skip_meta_key( $meta_key ) {
		return 'searchwp_' === substr( $meta_key, 0, strlen( 'searchwp_' ) ) || in_array( $meta_key, array(
			'_edit_lock',
			'_edit_last',
			'_wp_old_slug',
			'_searchwp_attempts',
	...
	function purge_post_via_update_post_metadata( $return, $object_id, $meta_key, $meta_value, $prev_value ) {
		...
		if ( $this->maybe_skip_meta_key( $meta_key ) || 'attachment' !== get_post_type( $object_id ) ) {
			return $return;
		}
	...
	function purge_post_via_edit_meta( $meta_id, $object_id, $meta_key, $_meta_value ) {
		...
		delete_post_meta( $object_id, SEARCHWP_PREFIX . 'content_pdf_metadata' );
