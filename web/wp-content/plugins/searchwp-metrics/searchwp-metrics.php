...
Copyright 2017-2025 SearchWP, LLC
...
define( 'SEARCHWP_METRICS_VERSION',    '1.4.8' );
...
if ( ! version_compare( PHP_VERSION, '5.6', '<' ) ) {
	...
	include_once SEARCHWP_METRICS_PLUGIN_DIR . '/includes/Upgrade.php';

	// Handle activation.
	register_activation_hook( __FILE__, 'searchwp_metrics_activate' );

	$searchwp_metrics = new SearchWP_Metrics();
	add_action( 'init', [ $searchwp_metrics, 'init' ] );
 ...
 * @since 1.4.8
 */
function searchwp_metrics_activate() {

	$upgrade = new \SearchWP_Metrics\Upgrade( SEARCHWP_METRICS_VERSION );
	$upgrade->maybe_install_tables();
}
