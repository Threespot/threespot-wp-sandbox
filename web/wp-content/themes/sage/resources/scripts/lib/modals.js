//------------------------------------------------------------------------
// Modal windows (customized for the color info modals)
//
...
export default class Modal extends EventEmitter {
  constructor(el, opts) {
    ...
    this.options = Object.assign(
      {},
      {
        ...
        bodyActiveClass: '',// string, only supports single class
        modalContentClass: null, // string, optional (defaults to first child)
        freezeScroll: true,// prevent scrolling when modal is expanded
    ...
    // Check if a custom selector was provided for the modal content
    this.customContentEl = this.options.modalContentClass ? this.el.querySelector('.' + this.options.modalContentClass) : null;
    this.contentEl = this.customContentEl !== null ? this.customContentEl : this.el.firstElementChild;
    ...
    if (
      ...
      if (this.el.hasAttribute('data-label')) {
        this.el.setAttribute('aria-label', this.el.dataset.label);
      } else {
        console.warn(
  ...
  destroy() {
    ...
    if (this.options.bodyActiveClass.length) {
  ...
  windowClickHandler(evt) {
    ...
    let isToggle = this.hasToggles && Array.prototype.indexOf.call(this.toggleEls, evt.target.closest('[data-modal]')) > -1;
    ...
    let targetInsideWrapper = this.contentEl.contains(evt.target);

    // Don’t close if target is the modal wrapper itself
    let targetIsWrapper = this.contentEl.isSameNode(evt.target);
    ...
    if (targetInsideWrapper && targetIsLink) {
      this.close();// Note: Don’t pass “evt” to avoid preventinf default o nlink clicks
    }
  }
  ...
  close(evt) {
    if (typeof evt !== 'undefined') {
      // Prevent default to stop browser from smooth scrolling to toggle button
      evt.preventDefault();
    }
